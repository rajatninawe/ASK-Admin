...
export class BaseService<T extends Id> {
  ...
  addRecepie(item: any) {
    return this.baseCollection.add(<T>(<unknown>{
      name: item.name,
  ...
  updateRecepie(item: any): Promise<void> {
    return this.db.doc(`/${this.path}/${item.docId}`).update(<T>(<unknown>{
      name: item.name,
      description: item.description,
      fish: item.fish,
      ingredients: item.ingredients,
